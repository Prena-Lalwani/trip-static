...
export default function TripPlanPanel({ plan = null }) {
  ...
  if (!plan || !plan.days?.length) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-center p-6 bg-zinc-950">
        <span className="text-4xl mb-3">✈️</span>
        <p className="text-zinc-300 font-medium text-sm">Trip plan coming soon</p>
  ...
  return (
    <div className="flex flex-col h-full bg-zinc-950">
      ...
      <div className="px-4 py-2.5 border-b border-zinc-800 flex items-center justify-between flex-shrink-0">
        <span className="text-xs text-zinc-400">{plan.trip_theme ?? 'Trip'} · ready to book</span>
      ...
      <div className="flex-1 overflow-y-auto flex flex-col">
        ...
        {/* Meta */}
        <div className="px-4 py-3 space-y-2 border-b border-zinc-800 flex-shrink-0">
          ...
          {plan.budget_per_person && (
            <p className="text-xs text-zinc-500">${plan.budget_per_person}/person · {days.length} days</p>
          )}
        </div>

        {/* Day tabs */}
        <div className="px-4 pb-3 border-b border-zinc-800 pt-3 flex-shrink-0">
          <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-2">Itinerary</p>
            ...
            {days.map(d => (
              <button
                ...
                className={`flex-shrink-0 w-8 h-8 rounded-full text-xs font-bold transition-colors ${
                  selectedDay === d.day_number ? 'bg-orange-500 text-white' : 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700'
                }`}
              >
                D{d.day_number}
              </button>
          ...
          {currentDay && (
            <div className="mt-2">
              <p className="text-[10px] font-bold text-zinc-500">DAY {currentDay.day_number}</p>
              <p className="text-xs text-zinc-300 mt-0.5">{currentDay.summary}</p>
            </div>
          )}
        </div>
        ...
        <div className="flex-shrink-0">
          <ExpandSection label="Places" expanded={expanded.places} onToggle={() => toggle('places')}>
            ...
            ) : <p className="text-xs text-zinc-500">No places listed for this day.</p>}
          </ExpandSection>
          ...
          <ExpandSection label="Activities" expanded={expanded.activities} onToggle={() => toggle('activities')}>
            ...
            ) : <p className="text-xs text-zinc-500">No activities listed for this day.</p>}
          </ExpandSection>
                ...
                <div className="space-y-3">
                    ...
                    {days.map(d => {
                      ...
                      return (
                        <button key={d.day_number} onClick={() => setSelectedDay(d.day_number)} className="w-full flex items-center gap-2 group">
                          <span className={`text-[10px] w-6 flex-shrink-0 text-right font-mono ${isSelected ? 'text-orange-400' : 'text-zinc-500'}`}>D{d.day_number}</span>
                          <div className="flex-1 h-4 bg-zinc-800 rounded-sm overflow-hidden">
                            <div className={`h-full rounded-sm transition-all ${isSelected ? 'bg-orange-500' : 'bg-zinc-600 group-hover:bg-zinc-500'}`} style={{ width: `${pct}%` }} />
                          </div>
                          <span className={`text-[10px] w-10 text-right flex-shrink-0 font-mono ${isSelected ? 'text-orange-400 font-bold' : 'text-zinc-500'}`}>${total}</span>
                        </button>
                      )
                    })}
                  </div>
                  {Object.keys(breakdown).length > 0 && (
                    <div className="border-t border-zinc-800 pt-2.5 space-y-1.5">
                      <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-wide mb-1">Day {selectedDay} breakdown</p>
                      {['transport', 'food', 'activities', 'accommodation'].map(key => (
                        breakdown[key] != null && (
                          <div key={key} className="flex items-center justify-between text-xs">
                            <span className="text-zinc-400 capitalize">{key}</span>
                            <span className="text-zinc-300">${breakdown[key]}</span>
                          </div>
                        )
                      ))}
                      <div className="flex items-center justify-between text-xs pt-1.5 border-t border-zinc-700 mt-1">
                        <span className="text-zinc-300 font-semibold">Day Total</span>
        ...
        {allDayBudgets > 0 && (
          <div className="px-4 py-3 border-t border-zinc-800 flex items-center justify-between flex-shrink-0">
            <span className="text-xs text-zinc-400">Trip total</span>
            <span className="text-sm font-bold text-white">${allDayBudgets.toLocaleString()}</span>
          </div>
        )}
      </div>

      <div className="flex-shrink-0 border-t border-zinc-800 p-3 flex items-center gap-3">
        ...
        <button className="text-sm text-zinc-400 hover:text-zinc-200 transition-colors">Share</button>
      </div>
    </div>
  )
}
